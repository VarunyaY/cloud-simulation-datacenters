...
/* In this file implemented a basic simulation by considering CloudSim, DatacenterBrokerSimple, DatacenterSimple
VmSimple, HostSimple classes.

CloudletScheduler with TimeShared policy, a simple VM allocation policy with full utilization by a cloudlet were
incorporated for this simulation.
...
//  Creates a list of Cloudlets which are the tasks from the customer.
  ...
  def createDatacenter(numberHosts: Int): DatacenterSimple = {
    ...
    val dc = new DatacenterSimple(simulation, hostList_new.asJava, new VmAllocationPolicySimple)
  ...
  def createCloudlets: util.List[CloudletSimple] = {
    ...
    of its execution. Considered full utilization model for this simulation.*/
